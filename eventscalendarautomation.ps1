...
# The script will prefer Managed Identity over storage account key for better security
# Required RBAC role for Managed Identity: "Storage Blob Data Contributor" on the Storage Account
# Logs are stored in $web container under "logs" folder for static website viewing
$StorageAccountName = "<YourStorageAccountName>"
$StorageAccountKey = "<YourStorageAccountKey>"  # Optional - will use Managed Identity if not provided
...
# Storage for verbose logs when running in Azure Automation
$Global:VerboseLogs = @()

# Address Type Caching for Performance Optimization
# These caches prevent redundant API calls for the same addresses across multiple events
$Global:AddressCache_Users = @{}                    # Confirmed individual users: email -> user object
$Global:AddressCache_ExchangeGroups = @{}           # Confirmed Exchange distribution groups: email -> group object
$Global:AddressCache_GraphGroups = @{}              # Confirmed M365 groups: email -> group object
$Global:AddressCache_NotUsers = @()                 # Confirmed NOT individual users
$Global:AddressCache_NotExchangeGroups = @()        # Confirmed NOT Exchange groups
$Global:AddressCache_NotGraphGroups = @()           # Confirmed NOT M365 groups
$Global:AddressCache_Stats = @{                     # Cache hit statistics
    UserHits = 0
    ExchangeGroupHits = 0
    GraphGroupHits = 0
    UserSkips = 0
    ExchangeGroupSkips = 0
    GraphGroupSkips = 0
    TotalLookups = 0
}

###########################################################################
# Function to display address cache performance statistics
###########################################################################
function Show-AddressCacheStats {
    Write-Log "📊 ADDRESS CACHE PERFORMANCE STATISTICS" -Level "INFO"
    Write-Log "════════════════════════════════════════" -Level "INFO"
    
    $Stats = $Global:AddressCache_Stats
    $TotalCacheHits = $Stats.UserHits + $Stats.ExchangeGroupHits + $Stats.GraphGroupHits
    $TotalCacheSkips = $Stats.UserSkips + $Stats.ExchangeGroupSkips + $Stats.GraphGroupSkips
    $TotalAPICalls = $Stats.TotalLookups - $TotalCacheHits - $TotalCacheSkips
    $CacheEfficiency = if ($Stats.TotalLookups -gt 0) { [Math]::Round(($TotalCacheHits + $TotalCacheSkips) / $Stats.TotalLookups * 100, 2) } else { 0 }
    
    Write-Log "Total Address Lookups: $($Stats.TotalLookups)" -Level "INFO"
    Write-Log "  ⚡ Cache Hits: $TotalCacheHits (found cached results)" -Level "INFO"
    Write-Log "    - Users: $($Stats.UserHits)" -Level "INFO"
    Write-Log "    - Exchange Groups: $($Stats.ExchangeGroupHits)" -Level "INFO"
    Write-Log "    - M365 Groups: $($Stats.GraphGroupHits)" -Level "INFO"
    Write-Log "  ⚡ Cache Skips: $TotalCacheSkips (avoided unnecessary calls)" -Level "INFO"
    Write-Log "    - Not Users: $($Stats.UserSkips)" -Level "INFO"
    Write-Log "    - Not Exchange Groups: $($Stats.ExchangeGroupSkips)" -Level "INFO"
    Write-Log "    - Not M365 Groups: $($Stats.GraphGroupSkips)" -Level "INFO"
    Write-Log "  🔍 API Calls Made: $TotalAPICalls" -Level "INFO"
    Write-Log "Cache Efficiency: $CacheEfficiency% (higher is better)" -Level "INFO"
    
    # Show cache contents summary
    Write-Log "Cache Contents Summary:" -Level "INFO"
    Write-Log "  - Cached Users: $($Global:AddressCache_Users.Count)" -Level "INFO"
    Write-Log "  - Cached Exchange Groups: $($Global:AddressCache_ExchangeGroups.Count)" -Level "INFO"
    Write-Log "  - Cached M365 Groups: $($Global:AddressCache_GraphGroups.Count)" -Level "INFO"
    Write-Log "  - Known Non-Users: $($Global:AddressCache_NotUsers.Count)" -Level "INFO"
    Write-Log "  - Known Non-Exchange Groups: $($Global:AddressCache_NotExchangeGroups.Count)" -Level "INFO"
    Write-Log "  - Known Non-M365 Groups: $($Global:AddressCache_NotGraphGroups.Count)" -Level "INFO"
    Write-Log "════════════════════════════════════════" -Level "INFO"
    
    if ($CacheEfficiency -gt 50) {
        Write-Log "🎉 Excellent cache performance! The script is running much faster thanks to caching." -Level "INFO"
    } elseif ($CacheEfficiency -gt 25) {
        Write-Log "👍 Good cache performance! Caching is providing decent speed improvements." -Level "INFO"
    } else {
        Write-Log "ℹ️ Cache is building up. Performance will improve with more events processed." -Level "INFO"
    }
}

###########################################################################
# Function to write logs to Azure Storage Blob
###########################################################################
function Write-LogsToStorageBlob {
    param (
        [Parameter(Mandatory=$true)]
        [array]$LogEntries
    )
    
    if (-not $IsRunningInAutomation -or $LogEntries.Count -eq 0) {
        return
    }
    
    try {
        # Create storage context - prefer Managed Identity over storage key
        $Context = $null
        
        # First try to use Managed Identity (if running in Azure Automation)
        if ($IsRunningInAutomation) {
            try {
                Write-Log "Attempting to connect to Azure Storage using Managed Identity" -Level "INFO"
                ...
                Write-Log "Successfully connected to Azure Storage using Managed Identity" -Level "INFO"
                Write-Log "🔐 AUTHENTICATION: Using Managed Identity" -Level "INFO"
            }
            catch {
                Write-Log "Failed to connect using Managed Identity: $($_.Exception.Message)" -Level "WARNING"
                Write-Log "Falling back to storage account key authentication" -Level "INFO"
                $Context = $null
            }
        }
        
        # Fallback to storage account key if Managed Identity failed or not in automation
        if (-not $Context) {
            $StorageKey = $StorageAccountKey
            if ([string]::IsNullOrEmpty($StorageKey)) {
                try {
                    $StorageKey = Get-AutomationVariable -Name "StorageAccountKey"
                    Write-Log "Retrieved storage account key from automation variables" -Level "INFO"
                }
                catch {
                    Write-Log "Could not retrieve storage account key from automation variables: $($_.Exception.Message)" -Level "WARNING"
                    Write-Log "Storage logging will be disabled" -Level "WARNING"
                    return
                }
            }
            
            if (-not [string]::IsNullOrEmpty($StorageKey)) {
                Write-Log "Creating storage context using storage account key" -Level "INFO"
                $Context = New-AzStorageContext -StorageAccountName $StorageAccountName -StorageAccountKey $StorageKey
                Write-Log "🔐 AUTHENTICATION: Using Storage Account Key" -Level "INFO"
            }
            else {
                Write-Log "No storage authentication method available - storage logging disabled" -Level "WARNING"
                return
            }
        }
        
        # Log successful authentication method after context creation
        if ($Context) {
            if ($Context.StorageAccountName -and $Context.ToString().Contains("DefaultEndpointsProtocol")) {
                Write-Log "🔐 FINAL AUTHENTICATION METHOD: Storage Account Key (Connection String)" -Level "INFO"
            } else {
                Write-Log "🔐 FINAL AUTHENTICATION METHOD: Managed Identity" -Level "INFO"
            }
        }
        
        # Ensure logs folder exists in the $web container (folder is created implicitly when we upload a blob to it)
        Write-Log "Using static website container '$LogContainerName' with logs folder '$LogFolderPath'" -Level "INFO"
        
        # Log the authentication method being used with more details
        $authMethod = "Unknown"
        $authDetails = ""
        
        if ($Context.StorageAccountName) {
            if ($Context.KeyName -or $Context.SasToken) {
                $authMethod = "Storage Account Key/SAS"
                $authDetails = "Using storage account access key or SAS token"
            } elseif ($Context.Context -and $Context.Context.GetType().Name -eq "AzureStorageContext") {
                # Check if this is a managed identity context
                try {
                    $contextProperties = $Context | Get-Member -MemberType Property
                    if ($contextProperties.Name -contains "TokenCredential" -or $Context.ToString().Contains("SharedAccessSignature") -eq $false) {
                        $authMethod = "Managed Identity"
                        $authDetails = "Using Azure Automation managed identity authentication"
                    } else {
                        $authMethod = "Storage Account Key"
                        $authDetails = "Using storage account access key authentication"
                    }
                } catch {
                    $authMethod = "Storage Account Key"
                    $authDetails = "Fallback authentication method detected"
                }
            } else {
                $authMethod = "Managed Identity"
                $authDetails = "Connected account authentication (managed identity)"
            }
        }
        
        Write-Log "🔐 Azure Storage Authentication Method: $authMethod" -Level "INFO"
        Write-Log "   Details: $authDetails" -Level "INFO"
        
        # Test connectivity to storage account
        try {
            Write-Log "Testing connectivity to storage account '$StorageAccountName'" -Level "INFO"
            Get-AzStorageBlob -Container $LogContainerName -Context $Context -MaxCount 1 -ErrorAction Stop | Out-Null
            Write-Log "Successfully connected to storage account - network access is working" -Level "INFO"
        }
        catch {
            Write-Log "Failed to connect to storage account: $($_.Exception.Message)" -Level "WARNING"
            Write-Log "This is likely due to network restrictions on the storage account" -Level "WARNING"
            Write-Log "Check Storage Account → Networking → Firewalls and virtual networks" -Level "INFO"
            return
        }
        
        # Create blob name with timestamp in the logs folder
        $BlobName = "$LogFolderPath/calendar-automation-log-$((Get-Date -Format 'yyyyMMdd-HHmmss')).log"
        ...
        try {
            ...
            Set-AzStorageBlobContent -File $TempFile -Container $LogContainerName -Blob $BlobName -Context $Context -Force -ErrorAction Stop | Out-Null
            
            Write-Information "Verbose logs uploaded to storage blob: $BlobName" -InformationAction Continue
        }
        catch {
            Write-Log "Failed to upload logs to blob '$BlobName': $($_.Exception.Message)" -Level "WARNING"
            Write-Log "This may be due to insufficient storage permissions. Logs will be available in Azure Automation job output only." -Level "WARNING"
        }
        finally {
            # Clean up temp file
        ...
        Write-Warning "Failed to upload logs to storage blob: $_"
    }
}

###########################################################################
# Function for logging - Azure Automation compatible with storage blob support
###########################################################################
function Write-Log {
    param (
        [Parameter(Mandatory=$true)]
    ...
    if ($IsRunningInAutomation) {
        # Add to verbose logs collection for storage blob
        $Global:VerboseLogs += $LogEntry
...
###########################################################################
# Optimized user and group lookup functions
###########################################################################
function Get-OptimizedUser {
    param (
        [Parameter(Mandatory=$true)]
        [string]$EmailAddress
    )
    ...
    $Global:AddressCache_Stats.TotalLookups++
    
    # Check if we know this is NOT a user
    if ($Global:AddressCache_NotUsers -contains $EmailAddress) {
        $Global:AddressCache_Stats.UserSkips++
        Write-Log "⚡ CACHE: Skipping user lookup for '$EmailAddress' (known NOT user)" -Level "INFO"
        return $null
    }
    
    # Check if we already have this user cached
    if ($Global:AddressCache_Users.ContainsKey($EmailAddress)) {
        $Global:AddressCache_Stats.UserHits++
        $CachedUser = $Global:AddressCache_Users[$EmailAddress]
        Write-Log "⚡ CACHE: Found cached user '$EmailAddress' -> $($CachedUser.DisplayName)" -Level "INFO"
        return $CachedUser
    }
    ...
    try {
        ...
        Write-Log "🔍 API: Looking up user '$EmailAddress'..." -Level "INFO"
        $User = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
            Get-MgUser -Filter "Mail eq '$EmailAddress' or userPrincipalName eq '$EmailAddress'" -Property "Id,DisplayName,Mail,UserPrincipalName,AccountEnabled,CreatedDateTime" -ErrorAction Stop
        }
        
        if ($User) {
            # Cache the positive result
            $Global:AddressCache_Users[$EmailAddress] = $User
            Write-Log "✅ CACHE: Stored user '$EmailAddress' -> $($User.DisplayName)" -Level "INFO"
        } else {
            # Cache the negative result
            $Global:AddressCache_NotUsers += $EmailAddress
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT a user" -Level "INFO"
        }
        
        return $User
    }
    catch {
        # Cache the negative result on error (likely not a user)
        $Global:AddressCache_NotUsers += $EmailAddress
        Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT a user (API error: $_)" -Level "WARNING"
        return $null
    }
}

###########################################################################
# Function to check if email is M365 Group (optimized)
###########################################################################
function Test-M365Group {
    param (
        [Parameter(Mandatory=$true)]
        [string]$EmailAddress
    )
    ...
    # Check if we know this is NOT an M365 group
    if ($Global:AddressCache_NotGraphGroups -contains $EmailAddress) {
        $Global:AddressCache_Stats.GraphGroupSkips++
        Write-Log "⚡ CACHE: Skipping M365 group lookup for '$EmailAddress' (known NOT M365 group)" -Level "INFO"
        return $false
    }
    
    # Check if we already have this M365 group cached
    if ($Global:AddressCache_GraphGroups.ContainsKey($EmailAddress)) {
        $Global:AddressCache_Stats.GraphGroupHits++
        $CachedGroup = $Global:AddressCache_GraphGroups[$EmailAddress]
        Write-Log "⚡ CACHE: Found cached M365 group '$EmailAddress' -> $($CachedGroup.DisplayName)" -Level "INFO"
        return $true
    }
    ...
    try {
        ...
        Write-Log "🔍 API: Looking up M365 group '$EmailAddress'..." -Level "INFO"
        $Group = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
            Get-MgGroup -Filter "mail eq '$EmailAddress'" -ErrorAction Stop
        }
        
        if ($Group) {
            # Cache the positive result
            $Global:AddressCache_GraphGroups[$EmailAddress] = $Group
            Write-Log "✅ CACHE: Stored M365 group '$EmailAddress' -> $($Group.DisplayName)" -Level "INFO"
            return $true
        } else {
            # Cache the negative result
            $Global:AddressCache_NotGraphGroups += $EmailAddress
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an M365 group" -Level "INFO"
            return $false
        }
    }
    catch {
        # Cache the negative result on error (likely not an M365 group)
        $Global:AddressCache_NotGraphGroups += $EmailAddress
        Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an M365 group (API error: $_)" -Level "WARNING"
        return $false
    }
}

###########################################################################
# Function to get group members with creation date for comparison (optimized for batch processing)
###########################################################################
function Get-GroupMembersWithCreatedDate {
    param (
        [Parameter(Mandatory=$true)]
        [string]$GroupEmail,
    ...
    try {
        Write-Log "Getting group members with creation dates for: $GroupEmail" -Level "INFO"
        ...
        if ($IsDistributionGroup) {
            Write-Log "Processing distribution group members for date comparison: $GroupEmail" -Level "INFO"
            
            # For distribution groups, we'll return all members since we can't get creation dates
            # from Exchange Online easily
            $DistGroupMembers = Get-DistributionGroupMembers -GroupEmail $GroupEmail
            
            return @{
                AllMembers = $DistGroupMembers
                NewMembers = $DistGroupMembers  # For dist groups, treat all as "new" to be safe
            ...
            # Find the M365 group (using optimized search)
            $Group = $null
            ...
            try {
                ...
                Write-Log "Could not find M365 group for date comparison: $GroupEmail" -Level "WARNING"
                return @{
                    AllMembers = @()
                    NewMembers = @()
            ...
            if ($Group) {
                Write-Log "Found M365 group for date comparison: $($Group.DisplayName)" -Level "INFO"
                
                # Get all group members in one call for better performance
                $GroupMembers = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
                ...
                # Get all member IDs first, then batch the user details requests
                $MemberIds = $GroupMembers | ForEach-Object { $_.Id }
                Write-Log "Processing $($MemberIds.Count) group members for date comparison" -Level "INFO"
                
                # Process members in smaller batches to avoid API limits but still be efficient
                $BatchSize = 10
                for ($i = 0; $i -lt $MemberIds.Count; $i += $BatchSize) {
                    $Batch = $MemberIds[$i..([Math]::Min($i + $BatchSize - 1, $MemberIds.Count - 1))]
                    
                    foreach ($MemberId in $Batch) {
                        try {
                            # Get member details with creation date
                            $MemberDetails = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
                                Get-MgUser -UserId $MemberId -Property "Id,DisplayName,Mail,UserPrincipalName,AccountEnabled,CreatedDateTime" -ErrorAction Stop
                            }
                            ...
                            if ($MemberDetails) {
                                ...
                                if ($IsValidEmail) {
                                    $AllMemberEmails += $EmailForEvent
                                    
                                    # Check if member was created after last run date
                                    if ($LastRunDate -and $MemberDetails.CreatedDateTime) {
                                        $MemberCreatedDate = [DateTime]::Parse($MemberDetails.CreatedDateTime)
                                        if ($MemberCreatedDate -gt $LastRunDate) {
                                            $NewMemberEmails += $EmailForEvent
                                            Write-Log "New member found: $EmailForEvent (Created: $($MemberCreatedDate.ToString('yyyy-MM-dd HH:mm:ss')))" -Level "INFO"
                                        }
                                    }
                                    else {
                                        # If no last run date or no creation date, treat as new
                                        $NewMemberEmails += $EmailForEvent
                                    }
                                }
                            }
                            ...
                            Write-Log "Error processing member for date comparison: $_" -Level "WARNING"
                        }
                    }
                    
                    # Small delay between batches to be nice to the API
                    if ($i + $BatchSize -lt $MemberIds.Count) {
                        Start-Sleep -Milliseconds 100
                    }
                }
                
                Write-Log "M365 Group '$($Group.DisplayName)' date comparison results:" -Level "INFO"
                Write-Log "  - Total members: $($AllMemberEmails.Count)" -Level "INFO"
                Write-Log "  - New members since last run: $($NewMemberEmails.Count)" -Level "INFO"
                
                return @{
                    AllMembers = $AllMemberEmails
                ...
                # Individual user
                return @{
                    AllMembers = @($GroupEmail)
                    NewMembers = @($GroupEmail)  # Individual users are always "new"
                    IsDistributionGroup = $false
                }
            }
        }
        ...
        Write-Log "Error getting group members with dates: $_" -Level "ERROR"
        return @{
            AllMembers = @()
            NewMembers = @()
            IsDistributionGroup = $false
...
###########################################################################
# Function to update DateOfLastRun in Excel
###########################################################################
function Update-DateOfLastRunInExcel {
    param (
        [Parameter(Mandatory=$true)]
        ...
        [Parameter(Mandatory=$true)]
        [hashtable]$ProcessedEvents  # Dictionary of event subjects to update dates
    )
    
    try {
        Write-Log "Updating DateOfLastRun column in Excel file" -Level "INFO"
        
        # Add DateOfLastRun column if it doesn't exist
        $UpdatedEventData = @()
        $CurrentDate = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        
        foreach ($EventItem in $EventData) {
            # Create a copy of the event
            $UpdatedEvent = $EventItem.PSObject.Copy()
            
            # Add DateOfLastRun property if it doesn't exist
            if ($UpdatedEvent.PSObject.Properties.Name -notcontains "DateOfLastRun") {
                $UpdatedEvent | Add-Member -MemberType NoteProperty -Name "DateOfLastRun" -Value $null
            }
            
            # Update DateOfLastRun if this event was processed
            if ($ProcessedEvents.ContainsKey($EventItem.Subject)) {
                $UpdatedEvent.DateOfLastRun = $CurrentDate
                Write-Log "Updated DateOfLastRun for event: $($EventItem.Subject)" -Level "INFO"
            }
        ...
        $UpdatedEventData | Export-Excel -Path $ExcelFilePath -WorksheetName "Sheet1" -ClearSheet -AutoSize
        Write-Log "Excel file updated with DateOfLastRun information" -Level "INFO"
        
        # Upload updated file to SharePoint
        try {
            Write-Log "Uploading updated Excel file with DateOfLastRun to SharePoint" -Level "INFO"
            ...
            Write-Log "Updated Excel file uploaded to SharePoint successfully" -Level "INFO"
        }
        catch {
        ...
        Write-Log "Error updating DateOfLastRun in Excel: $_" -Level "ERROR"
        return $EventData
    }
}
###########################################################################
# Function to classify errors as retriable or non-retriable
###########################################################################
function Test-ErrorRetriable {
    param (
        [Parameter(Mandatory=$true)]
        [string]$ErrorMessage,
    ...
    $ErrorMessageLower = $ErrorMessage.ToLower()
    
    # Define non-retriable error patterns by category
    $NonRetriablePatterns = @{
        # Exchange Online specific errors (Distribution Groups)
        "Exchange" = @(
        ...
        "Graph" = @(
            ...
            "invalid expression caused by operator",
            ...
            "invalid expression caused by operator 'equalsmatch'",
            "operator 'equalsmatch'"
        )
        
        # General authentication and permission errors (never retry these)
        "Auth" = @(
            "unauthorized",
            "access denied",
            "forbidden",
    ...
    # Define retriable error patterns (these should always be retried)
    $RetriablePatterns = @(
        "throttled",
        "rate limit",
        "too many requests",
    ...
    # First check if it's explicitly retriable (takes precedence)
    foreach ($pattern in $RetriablePatterns) {
        if ($ErrorMessageLower -match $pattern) {
            Write-Log "Error classified as RETRIABLE due to pattern '$pattern': $ErrorMessage" -Level "INFO"
            return $true
        }
    }
    
    # Check operation-specific non-retriable patterns
    switch ($OperationType) {
        "Exchange" {
            foreach ($pattern in $NonRetriablePatterns["Exchange"]) {
                if ($ErrorMessageLower -match $pattern) {
                    Write-Log "Error classified as NON-RETRIABLE (Exchange) due to pattern '$pattern': $ErrorMessage" -Level "INFO"
                    return $false
                }
            }
        }
        "Graph" {
            foreach ($pattern in $NonRetriablePatterns["Graph"]) {
                if ($ErrorMessageLower -match $pattern) {
                    Write-Log "Error classified as NON-RETRIABLE (Graph) due to pattern '$pattern': $ErrorMessage" -Level "INFO"
                    return $false
                }
            }
        }
    ...
    # Check general non-retriable patterns
    foreach ($category in @("Auth", "ClientError")) {
        foreach ($pattern in $NonRetriablePatterns[$category]) {
            if ($ErrorMessageLower -match $pattern) {
                Write-Log "Error classified as NON-RETRIABLE ($category) due to pattern '$pattern': $ErrorMessage" -Level "INFO"
                return $false
            }
        }
    }
    
    # If no specific pattern matched, default to retriable for safety
    Write-Log "Error not matching any specific pattern, defaulting to RETRIABLE: $ErrorMessage" -Level "INFO"
    return $true
}
...
function Invoke-WithRetry {
    ...
    while (-not $Completed -and $RetryCount -le $MaxRetries) {
        try {
            ...
            # Check if this error is retriable before attempting retry
            $IsRetriable = Test-ErrorRetriable -ErrorMessage $ErrorMessage -OperationType $OperationName
            
            if ($RetryCount -le $MaxRetries -and $IsRetriable) {
                Write-Log "$OperationName failed with error: $ErrorMessage. Error is retriable - retrying..." -Level "WARNING"
            }
            elseif (-not $IsRetriable) {
                Write-Log "$OperationName failed with non-retriable error: $ErrorMessage. Skipping retries." -Level "ERROR"
                throw $_
            }
            else {
...
# Add Azure modules if running in Azure Automation (for blob logging with managed identity)
if ($IsRunningInAutomation) {
...
function Get-FileFromSharePointGraph {
    ...
    try {
        ...
        Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
        ...
        foreach ($EventItem in $EventData) {
            try {
                ...
                if ($EventItem.StartTime) {
                    ...
                    if ($EventItem.StartTime -match '^\d{1,2}/\d{1,2}/\d{4}$' -or $EventItem.StartTime -match '^\d{4}-\d{1,2}-\d{1,2}$') {
                        # Date-only format
                        $EventStartDate = [DateTime]::Parse($EventItem.StartTime).Date
                    } else {
                    ...
                    if ($EventStartDate -lt $Today) {
                        Write-Log "Removing old event: '$($EventItem.Subject)' (Start: $($EventItem.StartTime))" -Level "INFO"
                        $RemovedEventsCount++
                    } else {
                        # Keep this event
                        $EventsToKeep += $EventItem
                    }
                } else {
                    Write-Log "Event '$($EventItem.Subject)' has no StartTime, keeping it for manual review" -Level "WARNING"
                    $EventsToKeep += $EventItem
                }
            }
            catch {
                Write-Log "Error parsing date for event '$($EventItem.Subject)': $_. Keeping event for manual review" -Level "WARNING"
                $EventsToKeep += $EventItem
            }
        }
...
try {
    ...
    $downloadedPath = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
...
function Test-ExistingCalendarEvent {
    ...
    try {
        $ExistingEvents = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
...
function Remove-EventsForUser {
    ...
    try {
        ...
        $DeletedCount = 0
        foreach ($EventItem in $EventsToDelete) {
            try {
                $Success = Remove-CalendarEventForUser -UserEmail $UserEmail -EventId $EventItem.Id
                if ($Success) {
                    $DeletedCount++
                    Write-Log "Deleted event: '$($EventItem.Subject)' (ID: $($EventItem.Id)) for user: $UserEmail" -Level "INFO"
                } else {
                    Write-Log "Failed to delete event: '$($EventItem.Subject)' (ID: $($EventItem.Id)) for user: $UserEmail" -Level "WARNING"
                }
            }
            catch {
                Write-Log "Error deleting event '$($EventItem.Subject)' for user $UserEmail`: $($_.Exception.Message)" -Level "ERROR"
            }
        }
...
function New-CalendarEventForUser {
    ...
    try {
        ...
        # Lookup the user object in Graph using optimized lookup
        Write-Log "Looking up user with email: $UserEmail" -Level "INFO"
        $User = Get-OptimizedUser -EmailAddress $UserEmail
    ...
    try {
        foreach ($GroupEmail in $AttendeeEmails) {
            ...
            if ($IsDistributionGroup) {
                ...
                # Get members of the distribution group using optimized lookup
                $DistGroupMembers = Get-GroupMembersWithCreatedDate -GroupEmail $GroupEmail -LastRunDate $null
                
                if ($DistGroupMembers -and $DistGroupMembers.AllMembers.Count -gt 0) {
                    Write-Log "Creating calendar events for distribution group members..." -Level "INFO"
                    $SuccessfulCreations = 0
                    $FailedCreations = 0
                    
                    foreach ($MemberEmail in $DistGroupMembers.AllMembers) {
                        try {
                            if ([string]::IsNullOrWhiteSpace($MemberEmail) -or ($MemberEmail -notmatch "@")) {
                    ...
                    Write-Log "  - Total members processed: $($DistGroupMembers.AllMembers.Count)" -Level "INFO"
                ...
                # Check if it's an M365 group using our cached function
                Write-Log "Checking if '$GroupEmail' is an M365 group..." -Level "INFO"
                $IsM365Group = Test-M365Group -EmailAddress $GroupEmail
                
                if ($IsM365Group) {
                    # Get the actual group object from cache
                    $Group = $Global:AddressCache_GraphGroups[$GroupEmail]
                    # We have an M365 group
                    ...
                    try {
                        ...
                        if ($GroupMembers) {
                            ...
                            # Use optimized group member lookup
                            $GroupMemberDetails = Get-GroupMembersWithCreatedDate -GroupEmail $GroupEmail -LastRunDate $null
                            $ValidMembers = @()
                            
                            if ($GroupMemberDetails -and $GroupMemberDetails.AllMembers.Count -gt 0) {
                                # Get member details in batches for efficiency
                                $BatchSize = 20
                                $MemberBatches = @()
                                for ($i = 0; $i -lt $GroupMemberDetails.AllMembers.Count; $i += $BatchSize) {
                                    $MemberBatches += ,@($GroupMemberDetails.AllMembers[$i..[Math]::Min($i + $BatchSize - 1, $GroupMemberDetails.AllMembers.Count - 1)])
                                }
                                
                                foreach ($Batch in $MemberBatches) {
                                    foreach ($MemberEmail in $Batch) {
                                        # Get additional member details using optimized lookup
                                        $MemberDetails = Get-OptimizedUser -EmailAddress $MemberEmail
                                        
                                        if ($MemberDetails) {
                                            $ValidMembers += @{
                                                EmailForEvent = $MemberEmail
                                                DisplayName = $MemberDetails.DisplayName
                                                AccountEnabled = $MemberDetails.AccountEnabled
                                            }
                                            Write-Log "✓ Valid member: $($MemberDetails.DisplayName) ($MemberEmail) [Enabled: $($MemberDetails.AccountEnabled)]" -Level "INFO"
                                        }
                                        else {
                                            Write-Log "⚠ Could not retrieve details for member: $MemberEmail" -Level "WARNING"
                                        }
                                    }
                                }
                            }
                            else {
                                # Fallback to direct member processing if group lookup fails
                                Write-Log "Group member lookup failed, falling back to direct member processing" -Level "WARNING"
                                ...
                                for ($i = 0; $i -lt $GroupMembers.Count; $i += $BatchSize) {
                                    $Batch = $GroupMembers[$i..([Math]::Min($i + $BatchSize - 1, $GroupMembers.Count - 1))]
                                        ...
                                        try {
                                            ...
                                            if ($MemberDetails) {
                                                ...
                                                if ($IsValidEmail) {
                                                    ...
                                                    $InvalidMembers += @{ DisplayName = $MemberDetails.DisplayName; IsValidEmail = $false }
...
function Test-DistributionGroup {
    ...
    # Check if we know this is NOT an Exchange distribution group
    if ($Global:AddressCache_NotExchangeGroups -contains $EmailAddress) {
        $Global:AddressCache_Stats.ExchangeGroupSkips++
        Write-Log "⚡ CACHE: Skipping Exchange group lookup for '$EmailAddress' (known NOT Exchange group)" -Level "INFO"
        return $false
    }
    
    # Check if we already have this Exchange group cached
    if ($Global:AddressCache_ExchangeGroups.ContainsKey($EmailAddress)) {
        $Global:AddressCache_Stats.ExchangeGroupHits++
        $CachedGroup = $Global:AddressCache_ExchangeGroups[$EmailAddress]
        Write-Log "⚡ CACHE: Found cached Exchange group '$EmailAddress' -> $($CachedGroup.DisplayName)" -Level "INFO"
        return $true
    }
    ...
    try {
        ...
        if ($IsM365Group) {
            ...
            # Cache as NOT an Exchange group
            $Global:AddressCache_NotExchangeGroups += $EmailAddress
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an Exchange group (is M365 group)" -Level "INFO"
            return $false
        }
        
        Write-Log "🔍 API: Looking up Exchange distribution group '$EmailAddress'..." -Level "INFO"
        
        # Ensure Exchange Online connection
        Assert-ExchangeAuth
        
        $DistGroup = Invoke-WithRetry -OperationName "Exchange" -ScriptBlock {
            Get-DistributionGroup -Identity $EmailAddress -ErrorAction Stop
        }
        
        if ($DistGroup) {
            # Cache the positive result
            $Global:AddressCache_ExchangeGroups[$EmailAddress] = $DistGroup
            Write-Log "✅ CACHE: Stored Exchange group '$EmailAddress' -> $($DistGroup.DisplayName)" -Level "INFO"
            return $true
        } else {
            # Cache the negative result
            $Global:AddressCache_NotExchangeGroups += $EmailAddress
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an Exchange group" -Level "INFO"
            return $false
        }
    }
    catch {
        # Cache the negative result on error
        $Global:AddressCache_NotExchangeGroups += $EmailAddress
        ...
        if ($_.Exception.Message -like "*not supported on GroupMailbox*") {
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an Exchange group (GroupMailbox error)" -Level "WARNING"
        } else {
            Write-Log "❌ CACHE: Stored '$EmailAddress' as NOT an Exchange group (API error: $_)" -Level "INFO"
        }
        return $false
    }
}
...
function Get-DistributionGroupMembers {
    ...
    try {
        ...
        $GroupMembers = Invoke-WithRetry -OperationName "Exchange" -ScriptBlock {
    ...
    try {
        ...
        if ($MissingPermissions.Count -gt 0) {
            ...
            try {
                ...
                $CurrentUser = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
                ...
                if ($CurrentUser) {
                    ...
                    $Calendar = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
...
function Test-CalendarEventsVisibility {
    ...
    try {
        ...
        $Events = Invoke-WithRetry -OperationName "Graph" -ScriptBlock {
...
$ProcessedEvents = @{}  # Track which events were processed for DateOfLastRun update

Write-Log "Starting to process $($EventData.Count) events from Excel"

foreach ($CalEvent in $EventData) {
    try {
        # Skip empty rows or rows with no subject
        if ([string]::IsNullOrWhiteSpace($CalEvent.Subject)) {
            Write-Log "Skipping empty event row (no subject)" -Level "WARNING"
            continue
        }
        
        # Skip rows with no start time
        if ([string]::IsNullOrWhiteSpace($CalEvent.StartTime)) {
            Write-Log "Skipping event '$($CalEvent.Subject)' - no start time specified" -Level "WARNING"
            continue
        }
        ...
        if ($ShouldDelete) {
            ...
            try {
                ...
                if ($DeletionResult.TotalDeletions -gt 0) {
                    ...
                    $ProcessedEvents[$CalEvent.Subject] = $true  # Mark as processed for DateOfLastRun update
                } else {
        ...
        # Check DateOfLastRun logic for existing events
        $LastRunDate = $null
        $ShouldCreateForNewMembersOnly = $false
        
        if ($CalEvent.PSObject.Properties.Name -contains "DateOfLastRun" -and 
            -not [string]::IsNullOrWhiteSpace($CalEvent.DateOfLastRun)) {
            
            try {
                $LastRunDate = [DateTime]::Parse($CalEvent.DateOfLastRun)
                $ShouldCreateForNewMembersOnly = $true
                Write-Log "Event '$($CalEvent.Subject)' was last run on: $($LastRunDate.ToString('yyyy-MM-dd HH:mm:ss'))" -Level "INFO"
                Write-Log "Will check for new group members since last run date" -Level "INFO"
            }
            catch {
                Write-Log "Invalid DateOfLastRun format for event '$($CalEvent.Subject)': $($CalEvent.DateOfLastRun)" -Level "WARNING"
                Write-Log "Treating as new event" -Level "INFO"
            }
        }
        else {
            Write-Log "Event '$($CalEvent.Subject)' has no previous run date - treating as new event" -Level "INFO"
        }
        ...
        try {
            ...
            # Handle dates without time components (like "All Hands" events)
            if ($CalEvent.StartTime -match '^\d{1,2}/\d{1,2}/\d{4}$' -or $CalEvent.StartTime -match '^\d{4}-\d{1,2}-\d{1,2}$') {
                if ($IsAllDay) {
        ...
        # Attendees - with new logic for checking new members only
        if ([string]::IsNullOrWhiteSpace($CalEvent.AttendeeEmails)) {
        ...
        $AttendeeEmails = $CalEvent.AttendeeEmails -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }
        if ($AttendeeEmails.Count -eq 0) {
            Write-Log "No valid attendee emails found for event: $($CalEvent.Subject)" -Level "WARNING"
            $FailureCount++
            continue
        }
        
        # Determine which attendees to process based on DateOfLastRun logic
        $FinalAttendeeEmails = @()
        $NewMembersFound = $false
        
        if ($ShouldCreateForNewMembersOnly) {
            Write-Log "Checking for new members in groups since last run date..." -Level "INFO"
            
            foreach ($AttendeeEmail in $AttendeeEmails) {
                $MemberInfo = Get-GroupMembersWithCreatedDate -GroupEmail $AttendeeEmail -LastRunDate $LastRunDate
                
                if ($MemberInfo.NewMembers.Count -gt 0) {
                    Write-Log "Found $($MemberInfo.NewMembers.Count) new members in $AttendeeEmail since last run" -Level "INFO"
                    $FinalAttendeeEmails += $AttendeeEmail  # Include this group/email for processing
                    $NewMembersFound = $true
                }
                else {
                    Write-Log "No new members found in $AttendeeEmail since last run - skipping" -Level "INFO"
                }
            }
            
            if (-not $NewMembersFound) {
                Write-Log "No new members found for event '$($CalEvent.Subject)' - skipping event creation" -Level "INFO"
                continue
            }
            
            Write-Log "Will create events for groups with new members: $($FinalAttendeeEmails -join ', ')" -Level "INFO"
        }
        else {
            # First time running this event - use all attendees
            $FinalAttendeeEmails = $AttendeeEmails
            Write-Log "First time processing event '$($CalEvent.Subject)' - will process all attendees" -Level "INFO"
        }
        ...
        Write-Log "  Final Attendees: $($FinalAttendeeEmails -join ', ')" -Level "INFO"
        
        # Create event for group members or individuals (only for new members if applicable)
        ...
        if ($Result) {
            $SuccessCount++
            $ProcessedEvents[$CalEvent.Subject] = $true  # Mark as processed for DateOfLastRun update
            Write-Log "Successfully processed event: $($CalEvent.Subject)" -Level "INFO"
        } else {
            $FailureCount++
...
# Update DateOfLastRun for processed events
try {
    if ($ProcessedEvents.Count -gt 0) {
        Write-Log "Updating DateOfLastRun for $($ProcessedEvents.Count) processed events" -Level "INFO"
        $EventData = Update-DateOfLastRunInExcel -ExcelFilePath $LocalExcelPath -EventData $EventData -ProcessedEvents $ProcessedEvents
    }
    else {
        Write-Log "No events were processed - skipping DateOfLastRun update" -Level "INFO"
    }
}
catch {
    Write-Log "Error updating DateOfLastRun: $_" -Level "WARNING"
}
...
try {
    ...
    # Upload verbose logs to storage blob if running in Azure Automation
    if ($IsRunningInAutomation -and $Global:VerboseLogs.Count -gt 0) {
        Write-Log "Uploading verbose logs to Azure Storage Blob" -Level "INFO"
        Write-Log "Total log entries to upload: $($Global:VerboseLogs.Count)" -Level "INFO"
        Write-LogsToStorageBlob -LogEntries $Global:VerboseLogs
        Write-Log "Storage blob upload process completed" -Level "INFO"
    }
}
catch {
